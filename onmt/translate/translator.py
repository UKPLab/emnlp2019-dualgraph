...
class Translator(object):
    ...
    def translate(
        ...
        if not src_node1:
            ...
            data = inputters.Dataset(
                self.fields,
                readers=([self.src_reader, self.src_reader, self.src_reader, self.tgt_reader]
                         if tgt else [self.src_reader, self.src_reader, self.src_reader]),
                data=[("src", src), ("node1", src_node1), ("node2", src_node2), ("tgt", tgt)] if tgt else
                [("src", src), ("node1", src_node1), ("node2", src_node2)],
    ...
    def _run_encoder(self, batch):
        ...
        if hasattr(self.model.encoder, 'is_graph_encoder'):
            enc_states, memory_bank, lengths = self.model.encoder(src, src_lengths, batch)
        else:
