...
def build_save_dataset(corpus_type, fields, src_reader, tgt_reader, opt):
    ...
    if corpus_type == 'train':
        ...
        if opt.data_type == "graph":
            node1s = opt.train_node1
            node2s = opt.train_node2
        ...
        if opt.data_type == "graph":
            node1s = [opt.valid_node1]
            node2s = [opt.valid_node2]

    if opt.data_type == "graph":
        zips = zip(srcs, node1s, node2s, tgts, ids)
    else:
        zips = zip(srcs, tgts, ids)

    for idx, z in enumerate(zips):
        if opt.data_type == "graph":
            src, node1, node2, tgt, maybe_id = z
            logger.info("Reading source, node1, node2 and target files: \n %s \n %s \n %s \n %s." % (src, node1, node2, tgt))
        ...
        if opt.data_type == "graph":
            ...
            for i, (src_shard, node1_shard, node2_shard, tgt_shard) in enumerate(shard_pairs):
                ...
                assert len(src_shard) == len(tgt_shard) == \
                       len(node1_shard) == len(node2_shard)

                logger.info("Building shard %d." % i)

                dataset = inputters.Dataset(
                    fields,
                    readers=([src_reader, src_reader, src_reader, tgt_reader]
                             if tgt_reader else [src_reader]),
                    data=([("src", src_shard), ("node1", node1_shard),
                           ("node2", node2_shard), ("tgt", tgt_shard)]
...
def main(opt):
    ...
    for src, tgt in zip(opt.train_src, opt.train_tgt):
        src_nfeats += count_features(src) if opt.data_type in ('text', 'graph') \
            else 0
